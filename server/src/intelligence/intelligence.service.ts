...
export class IntelligenceService {
  ...
  health() {
    return {
      ...
      policy: "No mock substitution: provider failures are returned as unavailable states.",
    };
  }
  ...
  async entities() {
    return this.cached("world-bank:countries", 24 * 60 * 60_000, async () => {
      ...
      return rows.filter((row: Record<string, unknown>) => String((row.region as Record<string, unknown>)?.id) !== "NA").map((row: Record<string, unknown>) => {
  ...
  async news(query = "geopolitics", requestedLimit = 100) {
    ...
    return this.cached(cacheKey, 10 * 60_000, async () => {
  ...
  async conflicts(days = 180) {
    return this.cached("ucdp:candidate", 6 * 60 * 60_000, async () => {
      ...
      return rows.filter((row) => new Date(row.date_end).getTime() >= cutoff && !row.side_a.startsWith("XXX") && !row.side_b.startsWith("XXX")).map((row) => ({
  ...
  async economy(code: string, indicator = "NY.GDP.MKTP.CD") {
    return this.cached(`world-bank:${code}:${indicator}`, 24 * 60 * 60_000, async () => {
      ...
      return { indicator, entityCode: code.toUpperCase(), observations: Array.isArray(rows) ? rows.map((row: Record<string, unknown>) => ({ year: Number(row.date), value: row.value, unit: row.unit || null })) : [], provenance: { provider: "World Bank Indicators API", retrievedAt: new Date().toISOString(), sourceUrl: url, providerMetadata: meta } };
    });
  }
