...
export function SideNavigation() {
  const router = useRouter(); const setFocus = useGeoStore((s)=>s.setFocus); const setTopic = useGeoStore((s)=>s.setTopic); const {data: topics=[]} = useTopics();
  const selectContinent = (slug:string) => { const c=geoService.continents.find((item)=>item.slug===slug); if(c){setFocus(c.coordinates,"continent");router.push(`/continent/${slug}`)} };
  return <aside className="panel-surface absolute bottom-5 left-5 top-5 z-10 hidden w-60 flex-col overflow-hidden lg:flex">
    ...
    <div className="border-b border-white/10 p-4"><div className="eyebrow mb-2">Continents</div>{geoService.continents.map((c)=><button key={c.slug} onClick={()=>selectContinent(c.slug)} className="flex w-full items-center justify-between py-2 text-left text-xs text-[var(--muted)] hover:text-white"><span>{c.name}</span><span className="mono text-[8px]">{c.entityCount}</span></button>)}</div>
    ...
    <div className="flex items-center gap-2 border-t border-white/10 px-4 py-3 text-[9px] text-[var(--faint)]"><RadioTower size={12}/>LIVE UX · PROTOTYPE DATA</div>
  </aside>;
}
