...
export function AppHeader() {
  ...
  const mobileNavOpen = useGeoStore((state) => state.mobileNavOpen);
  const setMobileNavOpen = useGeoStore((state) => state.setMobileNavOpen);
  return <>
    <header className="fixed inset-x-0 top-0 z-50 flex h-16 items-center border-b border-white/10 bg-[rgba(8,10,12,.9)] px-4 backdrop-blur-xl md:px-6">
      ...
      <div className="hidden min-w-fit items-center gap-3 md:flex"><div className="flex items-center gap-2 border-l border-white/10 pl-4"><span className="status-dot h-1.5 w-1.5 rounded-full bg-[var(--green)]"/><span className="eyebrow">Live sources</span></div><button className="p-2 text-[var(--muted)] hover:text-white" aria-label="Activity status"><Radio size={17}/></button></div>
    ...
    {mobileNavOpen && <div className="fixed inset-x-0 top-14 z-40 border-b border-white/10 bg-[#0c0f12] p-4 md:hidden"><div className="grid grid-cols-3 gap-px bg-white/10">{links.map(([href, label]) => <Link onClick={() => setMobileNavOpen(false)} key={href} href={href} className="bg-[#0c0f12] p-4 text-center text-xs">{label}</Link>)}</div><div className="mt-3 flex items-center gap-2 text-[10px] text-[var(--muted)]"><Globe2 size={13}/> Interactive world · source-backed data</div></div>}
  </>;
}
