...
export function ErrorState({ retry }: { retry?: () => void }) {
  return <div className="border border-[rgba(184,93,89,.35)] bg-[rgba(184,93,89,.06)] p-6"><AlertTriangle size={20} className="text-[var(--red)]"/><h3 className="mt-3 text-sm">This source is currently unavailable.</h3><p className="mt-2 text-xs text-[var(--muted)]">No substitute records were generated. Retry the provider request.</p>{retry && <button onClick={retry} className="mt-4 flex items-center gap-2 text-xs text-[var(--accent)]"><RotateCcw size={13}/> Retry</button>}</div>;
}
